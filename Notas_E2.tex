...
\textit{Notaci\'on utilizada en esta gu\'ia}:
...
Supongamos que los \'atomos realizan \'unicamente peque\~nas oscilaciones $\vec{u}$ con respecto a sus posiciones promedio $\vec{R}$ tal que podemos aproximar la energ\'ia total por el siguiente potencial arm\'onico:\\

$$ U(\{ \vec{r}_{m, \alpha}\}) = U(\{ \vec{R}_{m, \alpha} + \vec{u}_{m, \alpha}\}) \approx U_{a}(\{ \vec{u}_{m, \alpha}\}) = U_{eq} + \frac{1}{2}\sum_{m,n,\alpha,\beta,i,j} \frac{\partial^{2}U}{\partial r^{i}_{m,\alpha}\partial r^{j}_{n,\beta}} u^{i}_{m, \alpha}u^{j}_{n, \beta}$$
